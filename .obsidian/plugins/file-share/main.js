      ...
      if (data.type === "checkOnline" && data.target === friend.publicKey) {
        if (data.online) {
          const requestData = JSON.stringify({
            ...
            filename: (_a = this.file) == null ? void 0 : _a.name
          });
          const dataSign = this.plugin.secure.signData(requestData);
          this.socket.send("request", {
            target: friend.publicKey,
            filename: (_b = this.file) == null ? void 0 : _b.name,
            signature: dataSign
          });
          new import_obsidian.Notice(
            `Request sent to ${friend.username} for file ${(_c = this.file) == null ? void 0 : _c.name}`
          );
        } else {
        ...
        if (data.accepted) {
          new import_obsidian.Notice(`File request accepted by ${friend.username}`);
          ...
          new import_obsidian.Notice(`File request declined by ${friend.username}`);
        }
      }
    };
  }
...
// src/main.ts
var FileSharePlugin = class extends import_obsidian7.Plugin {
  async onload() {
